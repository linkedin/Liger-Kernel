...
from ..data_utils import maybe_extract_prompt
from ..models import create_reference_model
from ..models import prepare_deepspeed
from ..models.utils import prepare_fsdp
from .callbacks import SyncRefModelCallback
from .dpo_config import DPOConfig
from .dpo_config import FDivergenceConstants
from .dpo_config import FDivergenceType
from .utils import RunningMoments
from .utils import cap_exp
from .utils import disable_dropout_in_model
from .utils import empty_cache
from .utils import flush_left
from .utils import flush_right
...
class DPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if args.use_liger_loss:
            ...
            if args.loss_type not in ["sigmoid", "apo_zero", "apo_down"]:
                raise ValueError(
                    f"You set `use_liger_loss=True` but the loss type is `{args.loss_type}`. "
                    "The Liger kernel currently supports only 'sigmoid', 'apo_zero', and 'apo_down' loss types."
                )
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        if self.precompute_ref_log_probs and not self._precomputed_train_ref_log_probs:
            ...
            self.train_dataset = self.train_dataset.add_column(name="ref_rejected_logps", column=all_ref_rejected_logps)
