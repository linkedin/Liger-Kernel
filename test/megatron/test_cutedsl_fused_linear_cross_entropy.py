...
from liger_kernel.ops.cutedsl.ops.megatron_fused_linear_cross_entropy import (  # noqa: E402
    liger_megatron_fused_linear_cross_entropy,
)

pytestmark = [
    pytest.mark.skipif(not torch.cuda.is_available(), reason="CuTe DSL FLCE requires CUDA"),
    pytest.mark.skipif(
        torch.cuda.is_available() and torch.cuda.get_device_capability()[0] < 10,
        reason="native CuTe DSL Megatron FLCE requires Blackwell",
    ),
]
...
def test_cutedsl_megatron_flce_tp1_matches_pytorch(dtype, with_bias):
    ...
    hidden_cutedsl = hidden_base.clone().requires_grad_(True)
    weight_cutedsl = weight_base.clone().requires_grad_(True)
    bias_cutedsl = bias_base.clone().requires_grad_(True) if bias_base is not None else None
    ...
    torch.testing.assert_close(hidden_cutedsl.grad, hidden_ref.grad, atol=5e-3, rtol=5e-2)
    torch.testing.assert_close(weight_cutedsl.grad, weight_ref.grad, atol=5e-3, rtol=5e-2)
    if with_bias:
        torch.testing.assert_close(bias_cutedsl.grad, bias_ref.grad, atol=5e-3, rtol=5e-2)


def test_cutedsl_megatron_flce_backend_dispatch():
    env = os.environ.copy()
    ...
    assert result.stdout.strip() == "liger_kernel.ops.cutedsl.ops.megatron_fused_linear_cross_entropy"
...
@pytest.mark.skipif(torch.cuda.device_count() < 2, reason="requires at least two CUDA GPUs")
@pytest.mark.parametrize("dtype", [torch.bfloat16, torch.float16])
def test_cutedsl_megatron_flce_tp2_matches_global_reference(dtype):
