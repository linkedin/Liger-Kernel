...
# Shapes exercise a range of H (256-aligned and non-aligned), odd V, and tail
# tokens through the cuBLAS dW GEMM.
# ---------------------------------------------------------------------------
...
@pytest.mark.parametrize(
    "shape",
    [
        (130, 128, 256),  # H=128, 130-row tail chunk
        (200, 96, 384),  # H=96 (non-aligned)
...
# ---------------------------------------------------------------------------
# GEMM ownership trace: the dW gradient now runs through PyTorch/cuBLAS, not a
# custom cuTile MMA kernel. The chunked path must emit exactly one FP32-out
# ``aten::mm`` (first chunk) and one FP32-out ``aten::addmm`` per subsequent
# chunk; projection and dX use the plain (BF16-out) ``aten::mm.out`` overload.
# The removed cuTile matmul kernels must no longer exist on the module.
# ---------------------------------------------------------------------------
...
def test_chunked_dw_uses_mm_then_addmm_fp32_out():
    torch.manual_seed(3)
    n_tokens, hidden, vocab, chunk_size = 70, 96, 512, 16  # 70 = 16*4 + 6 -> 5 chunks
    x = torch.randn(n_tokens, hidden, device="cuda", dtype=torch.bfloat16, requires_grad=True)
    weight = torch.randn(vocab, hidden, device="cuda", dtype=torch.bfloat16, requires_grad=True)
    target = torch.randint(vocab, (n_tokens,), device="cuda")

    n_chunks = len(range(0, n_tokens, chunk_size))
    with _AtenOpRecorder() as rec, torch.no_grad():
        chunked_fused_linear_cross_entropy_forward(x, weight, target, chunk_size=chunk_size)
    ...
    if flce_mod._ADDMM_SUPPORTS_OUT_DTYPE:
        ...
        # No custom cuTile MMA reaches torch dispatch (it never did); assert dW never
        # accumulated straight into a BF16 tensor via a plain addmm.
        ...
        # (projection/dX use mm.out, so mm.out alone can't isolate the dW GEMM).
...
# helper must upcast the bounded operands to FP32 and use the plain mm/addmm
# (no ``.dtype_out`` op). Small gradients must still survive the deferred cast.
# ---------------------------------------------------------------------------
def test_pre_2_8_fallback_avoids_out_dtype_overload(monkeypatch):
    ...
    torch.manual_seed(9)
    n_tokens, hidden, vocab, chunk_size = 50, 64, 256, 16  # multi-chunk
    x = torch.randn(n_tokens, hidden, device="cuda", dtype=torch.bfloat16, requires_grad=True)
    weight = torch.randn(vocab, hidden, device="cuda", dtype=torch.bfloat16, requires_grad=True)
    target = torch.randint(vocab, (n_tokens,), device="cuda")
