...
class HFKTOLoss(HFAlignmentLossKTO):
    ...
    def alignment_loss(
        ...
        if policy_chosen_logps.shape[0] != 0 or reference_chosen_logps.shape[0] != 0:
            chosen_logratios = policy_chosen_logps.sum(-1) - reference_chosen_logps.sum(
                -1
            )
            ...
            chosen_losses = torch.Tensor([]).to(self.device)
            chosen_rewards = torch.Tensor([]).to(self.device)
        ...
        if (
            ...
            rejected_logratios = policy_rejected_logps.sum(
                -1
            ...
            # lists can't be empty -- if they are, then accelerate.gather will hang
            rejected_losses = torch.Tensor([]).to(self.device)
            rejected_rewards = torch.Tensor([]).to(self.device)
...
@pytest.mark.parametrize(
    "ignore_index, beta, alpha", [(-100, 0.1, 1.0), (42, 0.2, 1.0)]
)
def test_correctness(
    ...
    loss1 = HFKTOLoss(
        ignore_index=ignore_index, beta=beta, device="cuda"
