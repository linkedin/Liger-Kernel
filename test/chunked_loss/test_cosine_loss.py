...
def test_cosine_frozen_student_weight(compiled, bias):
    B, T, H, V = 2, 8, 32, 64
...
@pytest.mark.parametrize("compiled", [False, True])
def test_cosine_no_grad_required(compiled):
    B, T, H, V = 2, 8, 32, 64
    torch.manual_seed(42)
    student_input = torch.randn(B * T, H, device=device, dtype=torch.float32)
    teacher_input = torch.randn(B * T, H, device=device, dtype=torch.float32)
...
def test_cosine_differentiates_only_params_that_require_grad(monkeypatch, weight_requires_grad):
