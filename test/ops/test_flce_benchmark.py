...
class TestCuTileProjectionGeometry:
    ...
    def test_projection_row_geometry_matches_chunking(self):
        ...
        # cuTile owns the CE stats / dZ / scale-cast; all GEMMs (projection, dX,
        # dW) run on torch BLAS. The projection (x_chunk @ w.T) is filtered by its
        # [H, V] RHS, so the dW mm (FP32-out) is not counted as a projection.
        proj = [lhs for lhs, rhs in rec.mms if rhs == (_HIDDEN, _VOCAB)]
        rows = [lhs[0] for lhs in proj]
