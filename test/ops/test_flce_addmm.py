...
numerical parity against a full-precision autograd reference. The whole file is
...
# ---------------------------------------------------------------------------
#
# The whole file is CUDA-gated by ``conftest.py``'s autouse fixture, but *being
# on CUDA* is not sufficient for the ``addmm(out=)`` / ``addmm.dtype_out`` fast
# paths. Production (``fused_linear_cross_entropy_forward``) gates them on:
#
#   * ``on_cuda_sm80``  -- ``get_device_capability()[0] >= 8`` (SM80+; this is
...
# Reused across every fast-path-asserting test (mirrors production's on_cuda_sm80).
...
def _run_flce(
    ...
    *,
    ...
    autocast, optionally under a dispatch recorder."""
...
def _mk_inputs(
    ...
    *,
    ...
    all_ignore=False,
    first_chunk_ignore=False,
    chunk_size=None,
):
    g = torch.Generator(device=DEVICE).manual_seed(seed)
    input_dtype = input_dtype or dtype
    _input = torch.randn(BT, H, device=DEVICE, dtype=input_dtype, generator=g) * 0.1
    _input.requires_grad_(input_requires_grad)
    weight = torch.randn(V, H, device=DEVICE, dtype=dtype, generator=g) * 0.1
    weight.requires_grad_(weight_requires_grad)
    b = None
    if bias:
        b = (torch.randn(V, device=DEVICE, dtype=dtype, generator=g) * 0.1).requires_grad_(True)
    cw = None
    if ce_weight:
        cw = torch.rand(V, device=DEVICE, dtype=torch.float32, generator=g) + 0.1
    target = torch.randint(0, V, (BT,), device=DEVICE, generator=g, dtype=torch.long)
    if all_ignore:
        target[:] = -100
    elif first_chunk_ignore and chunk_size is not None:
        target[:chunk_size] = -100
...
def test_lowprec_default_uses_addmm_out_no_param_upcast(dtype, bias):
    BT, V, H, cs = 40, 64, 32, 16  # 3 chunks (16,16,8) -> multichunk + tail
    _input, weight, b, cw, target = _mk_inputs(BT, V, H, dtype, bias=bias, ce_weight=False, chunk_size=cs)

    rec = _OpRecorder()
    loss, gi, gw, gb = _run_flce(_input, weight, target, bias=b, chunk_size=cs, accum_dtype=None, recorder=rec)
    ...
    assert rec.has_addmm_out_into(weight.shape), (
        f"expected aten.addmm.out into {tuple(weight.shape)}, saw {rec.addmm_out_shapes}"
    )
    ...
    tol = _TOLS[dtype]
    _, ref_gi, ref_gw, ref_gb = _reference_grads(_input, weight, target, bias=b)
...
def test_parity_combinations(dtype, reduction, bias, ce_weight, label_smoothing):
    BT, V, H, cs = 40, 64, 32, 16
    _input, weight, b, cw, target = _mk_inputs(BT, V, H, dtype, bias=bias, ce_weight=ce_weight, chunk_size=cs, seed=1)
    ...
    _, ref_gi, ref_gw, ref_gb = _reference_grads(
        _input,
        weight,
        target,
        bias=b,
...
@pytest.mark.parametrize("dtype", [torch.bfloat16, torch.float16])
def test_parity_reduction_none_nonuniform_upstream(dtype):
    BT, V, H, cs = 24, 48, 32, 8  # 3 chunks (8,8,8)
    _input, weight, b, cw, target = _mk_inputs(BT, V, H, dtype, bias=True, ce_weight=False, chunk_size=cs, seed=2)
    g = torch.Generator(device=DEVICE).manual_seed(7)
    token_grad_output = torch.rand(BT, device=DEVICE, dtype=torch.float32, generator=g) + 0.05
    _, gi, gw, gb = _run_flce(
        _input,
        weight,
        target,
...
@pytest.mark.parametrize("dtype", [torch.bfloat16, torch.float16])
def test_first_chunk_all_ignored(dtype):
    BT, V, H, cs = 40, 64, 32, 16
    _input, weight, b, cw, target = _mk_inputs(
        BT, V, H, dtype, bias=True, ce_weight=False, chunk_size=cs, seed=3, first_chunk_ignore=True
    )
    _, gi, gw, gb = _run_flce(_input, weight, target, bias=b, chunk_size=cs)
    _, ref_gi, ref_gw, ref_gb = _reference_grads(_input, weight, target, bias=b)
    tol = _TOLS[dtype]
    assert torch.isfinite(gw).all()
...
@pytest.mark.parametrize("dtype", [torch.bfloat16, torch.float16])
def test_all_tokens_ignored_zero_grad(dtype):
    BT, V, H, cs = 32, 64, 32, 16
    _input, weight, b, cw, target = _mk_inputs(
        BT, V, H, dtype, bias=True, ce_weight=False, chunk_size=cs, seed=4, all_ignore=True
    )
    _, gi, gw, gb = _run_flce(_input, weight, target, bias=b, chunk_size=cs)
...
def test_frozen_weight_trainable_input():
    BT, V, H, cs = 40, 64, 32, 16
    _input, weight, b, cw, target = _mk_inputs(
        BT,
        V,
    ...
    rec = _OpRecorder()
    _, gi, gw, gb = _run_flce(_input, weight, target, chunk_size=cs, recorder=rec)
    assert gw is None
    assert not rec.has_addmm_out_into(weight.shape)
    assert not rec.has_addmm_dtype_out_into(weight.shape)
    # grad_input must still be produced and correct.
    _, ref_gi, _, _ = _reference_grads(_input, weight, target)
    torch.testing.assert_close(gi.float(), ref_gi, **_TOLS[torch.bfloat16])
...
@pytest.mark.parametrize("dtype", [torch.bfloat16, torch.float16])
def test_noncontiguous_input_and_weight(dtype):
    BT, V, H, cs = 40, 64, 32, 16
    g = torch.Generator(device=DEVICE).manual_seed(6)
    _input = (torch.randn(H, BT, device=DEVICE, dtype=dtype, generator=g) * 0.1).t().requires_grad_(True)
    weight = (torch.randn(H, V, device=DEVICE, dtype=dtype, generator=g) * 0.1).t().requires_grad_(True)
    assert not _input.is_contiguous() and not weight.is_contiguous()
    target = torch.randint(0, V, (BT,), device=DEVICE, generator=g, dtype=torch.long)
    _, gi, gw, gb = _run_flce(_input, weight, target, chunk_size=cs)
    _, ref_gi, ref_gw, _ = _reference_grads(_input, weight, target)
    tol = _TOLS[dtype]
    assert torch.isfinite(gw).all()
    torch.testing.assert_close(gw.float(), ref_gw, **tol)
    torch.testing.assert_close(gi.float(), ref_gi, **tol)
...
@pytest.mark.parametrize("dtype", [torch.bfloat16, torch.float16])
def test_fp32_accum_uses_dtype_out_fastpath(dtype):
    BT, V, H, cs = 40, 64, 32, 16
    _input, weight, b, cw, target = _mk_inputs(BT, V, H, dtype, bias=False, ce_weight=False, chunk_size=cs, seed=8)
    rec = _OpRecorder()
    _, gi, gw, gb = _run_flce(_input, weight, target, accum_dtype=torch.float32, chunk_size=cs, recorder=rec)
    # Accumulator is fp32; the returned grad is cast back to the weight dtype.
    assert gw.dtype == weight.dtype
    # The fp32 out_dtype overload must be used, and the plain out overload must not.
    assert rec.has_addmm_dtype_out_into(weight.shape), (
        f"expected aten.addmm.dtype_out into {tuple(weight.shape)}, saw {rec.addmm_dtype_out_shapes}"
    )
    assert not rec.has_addmm_out_into(weight.shape)
    ...
    torch.testing.assert_close(gi.float(), ref_gi, **_TOLS[dtype])


# ---------------------------------------------------------------------------
# 7. Pre-2.8 torch (no out_dtype support) -> legacy fallback, never the out_dtype op
# ---------------------------------------------------------------------------


@requires_sm80
@pytest.mark.parametrize("accum_dtype", [None, torch.float32])
def test_pre28_falls_back_to_legacy_mm(monkeypatch, accum_dtype):
    BT, V, H, cs = 40, 64, 32, 16
    # Under bf16 params + fp32 accumulator, the fast path needs out_dtype support.
    _input, weight, b, cw, target = _mk_inputs(
        BT, V, H, torch.bfloat16, bias=False, ce_weight=False, chunk_size=cs, seed=9
    )
    monkeypatch.setattr(flce_ops, "_ADDMM_SUPPORTS_OUT_DTYPE", False)
    rec = _OpRecorder()
    _, gi, gw, gb = _run_flce(_input, weight, target, accum_dtype=accum_dtype, chunk_size=cs, recorder=rec)
    ...
    if accum_dtype is torch.float32:
        ...
        assert not rec.has_param_upcast(weight.shape)
    _, ref_gi, ref_gw, _ = _reference_grads(_input, weight, target)
    torch.testing.assert_close(gw.float(), ref_gw, **_TOLS[torch.bfloat16])
    torch.testing.assert_close(gi.float(), ref_gi, **_TOLS[torch.bfloat16])
...
@requires_sm80
def test_amp_bf16_params_fp32_input_uses_lowprec_addmm():
    ...
    _input = (torch.randn(BT, H, device=DEVICE, dtype=torch.float32, generator=g) * 0.1).requires_grad_(True)
    weight = (torch.randn(V, H, device=DEVICE, dtype=torch.bfloat16, generator=g) * 0.1).requires_grad_(True)
    target = torch.randint(0, V, (BT,), device=DEVICE, generator=g, dtype=torch.long)

    rec = _OpRecorder()
    _, gi, gw, gb = _run_flce(_input, weight, target, chunk_size=cs, recorder=rec, autocast_dtype=torch.bfloat16)
    assert gw.dtype == torch.bfloat16
    assert rec.has_addmm_out_into(weight.shape)
    assert not rec.has_addmm_dtype_out_into(weight.shape)
    assert not rec.has_param_upcast(weight.shape)
    _, ref_gi, ref_gw, _ = _reference_grads(_input, weight, target)
    tol = _TOLS[torch.bfloat16]
    torch.testing.assert_close(gw.float(), ref_gw, **tol)
    torch.testing.assert_close(gi.float(), ref_gi, **tol)
...
@requires_sm80
@requires_addmm_out_dtype
def test_amp_fp32_params_no_bias_uses_dtype_out_fastpath():
    ...
    _input = (torch.randn(BT, H, device=DEVICE, dtype=torch.float32, generator=g) * 0.1).requires_grad_(True)
    weight = (torch.randn(V, H, device=DEVICE, dtype=torch.float32, generator=g) * 0.1).requires_grad_(True)
    target = torch.randint(0, V, (BT,), device=DEVICE, generator=g, dtype=torch.long)
...
def test_amp_fp32_params_fp32_bias_promotes_and_falls_back():
    ...
    _input = (torch.randn(BT, H, device=DEVICE, dtype=torch.float32, generator=g) * 0.1).requires_grad_(True)
    weight = (torch.randn(V, H, device=DEVICE, dtype=torch.float32, generator=g) * 0.1).requires_grad_(True)
    bias = (torch.randn(V, device=DEVICE, dtype=torch.float32, generator=g) * 0.1).requires_grad_(True)
    target = torch.randint(0, V, (BT,), device=DEVICE, generator=g, dtype=torch.long)

    rec = _OpRecorder()
    _, gi, gw, gb = _run_flce(
        _input, weight, target, bias=bias, chunk_size=cs, recorder=rec, autocast_dtype=torch.bfloat16
    )
    # grad_logits promoted to fp32 -> no fp32-out fast path, no low-prec out path.
    assert not rec.has_addmm_dtype_out_into(weight.shape)
    assert not rec.has_addmm_out_into(weight.shape)
    assert rec.mm_default >= 1
    _, ref_gi, ref_gw, ref_gb = _reference_grads(_input, weight, target, bias=bias)
    torch.testing.assert_close(gw.float(), ref_gw, atol=1e-2, rtol=1e-2)
    torch.testing.assert_close(gi.float(), ref_gi, atol=1e-2, rtol=1e-2)
...
@requires_sm80
def test_amp_use_token_scaling_lowprec():
    ...
    BT, V, H, cs = 24, 48, 32, 8
    _input, weight, b, cw, target = _mk_inputs(
        BT, V, H, torch.bfloat16, bias=True, ce_weight=False, chunk_size=cs, seed=13
    )
    rec = _OpRecorder()
    _, gi, gw, gb = _run_flce(_input, weight, target, bias=b, chunk_size=cs, use_token_scaling=True, recorder=rec)
    assert gw.dtype == torch.bfloat16
    assert rec.has_addmm_out_into(weight.shape)
    assert not rec.has_param_upcast(weight.shape)
    assert torch.isfinite(gw).all() and torch.isfinite(gi).all()
...
def test_sm_capability_guard_forces_legacy_fallback(monkeypatch):
    ...
    BT, V, H, cs = 40, 64, 32, 16
    _input, weight, b, cw, target = _mk_inputs(
        BT, V, H, torch.bfloat16, bias=False, ce_weight=False, chunk_size=cs, seed=14
    )
