...
memory budget (``inc_factor = cdiv(V, C * H)``). At the tiny shapes used here the
default ``C=16`` collapses to a single chunk, which would never exercise the
per-chunk grad_weight accumulation these tests target. The autouse
``_force_multichunk`` fixture pins that existing budget knob to ``C=1`` so the
real production loop splits these shapes into multiple chunks -- we drive the
genuine loop rather than any test-only chunking API, and derive the chunk
geometry from the same formula production uses (``_production_chunk_size``).
"""
...
# ---------------------------------------------------------------------------
# Force the real production loop to multichunk at test shapes
# ---------------------------------------------------------------------------


@pytest.fixture(autouse=True)
def _force_multichunk(monkeypatch):
    ...
    several token chunks, so the genuine per-chunk grad_weight accumulation loop
    runs more than once -- which is exactly what these regression tests cover.
    """
...
def _production_chunk_size(BT: int, V: int, H: int) -> int:
    ...
    *current* module-level ``_CHUNK_MEM_CONST`` (pinned to 1 by the autouse
    fixture). Mirrors the arithmetic in ``fused_linear_cross_entropy_forward``."""
...
def test_first_chunk_all_ignored(dtype):
    ...
    _input, weight, b, cw, target = _mk_inputs(BT, V, H, dtype, bias=True, ce_weight=False, seed=3, first_chunk_size=cs)
