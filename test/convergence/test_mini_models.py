...
def run_mini_model(
    ...
    loss_list, hidden_states, attentions = [], [], []
...
def test_mini_model(
    ...
    expected_hidden = expected_output["hidden_states"]
    actual_hidden = actual_output["hidden_states"]

    expected_attentions = expected_output["attentions"]
    actual_attentions = actual_output["attentions"]

    [
        [torch.all(expected == actual) for expected, actual in zip(expected_ls, actual_ls)]
        for expected_ls, actual_ls in zip(expected_hidden, actual_hidden)
    ]
    [
        [torch.all(expected == actual) for expected, actual in zip(expected_ls, actual_ls)]
        for expected_ls, actual_ls in zip(expected_attentions, actual_attentions)
    ]
    # expected_output["loss"]
    # [11.118860244750977, 2.3487460613250732, 0.8708779811859131, 0.7115309238433838, 0.684813916683197, 0.7941077351570129, 0.7296597361564636, 0.9826794862747192, 0.6943902373313904, 0.7848423719406128, 0.7918236255645752, 0.48607656359672546, 0.7277156114578247, 0.7007701992988586, 0.6336098909378052, 0.7450151443481445, 0.7330642938613892, 0.7945504188537598, 0.7074819803237915, 0.7754990458488464, 0.6290615200996399, 0.3892846405506134, 0.4946531653404236, 0.46465131640434265, 0.4638962149620056, 0.4265981912612915, 0.3813912272453308, 0.5081480741500854, 0.6009138822555542, 0.38953331112861633, 0.7501488327980042, 0.48894357681274414]
    # actual_output["loss"]
    # [11.11886215209961, 2.3487491607666016, 0.8708779811859131, 0.711530864238739, 0.6848140358924866, 0.7941078543663025, 0.7296596169471741, 0.9826793670654297, 0.6943902373313904, 0.784842312335968, 0.7918239235877991, 0.4860764741897583, 0.727715790271759, 0.7007702589035034, 0.6336102485656738, 0.7450150847434998, 0.7330642342567444, 0.7945505380630493, 0.707482099533081, 0.775499165058136, 0.6290611624717712, 0.38928452134132385, 0.49465322494506836, 0.46465128660202026, 0.4638962745666504, 0.4265981614589691, 0.3813909590244293, 0.5081482529640198, 0.6009137630462646, 0.389533668756485, 0.7501487135887146, 0.488943487405777]

    # expected_output["loss"]
    # [10.6544771194458, 2.220482349395752, 0.8619861602783203, 0.6984319686889648, 0.6673918962478638, 0.7843921780586243, 0.7144726514816284, 0.9712896347045898, 0.6808816194534302, 0.7638488411903381, 0.7750442624092102, 0.47593384981155396, 0.7066197395324707, 0.6847841143608093, 0.6138404607772827, 0.7335494160652161, 0.7122305631637573, 0.7821023464202881, 0.692719042301178, 0.7584717869758606, 0.6214989423751831, 0.38992685079574585, 0.4882435202598572, 0.4643993675708771, 0.4571280777454376, 0.42835772037506104, 0.38260504603385925, 0.5120083689689636, 0.6006607413291931, 0.3842446804046631, 0.7511336803436279, 0.4866192936897278]
    # actual_output["loss"]
    # [10.654476165771484, 2.2204833030700684, 0.8619862794876099, 0.6984319090843201, 0.6673920750617981, 0.7843920588493347, 0.7144728899002075, 0.9712896943092346, 0.680881679058075, 0.7638488411903381, 0.7750440239906311, 0.47593387961387634, 0.706619918346405, 0.6847841739654541, 0.6138402819633484, 0.7335492968559265, 0.7122305631637573, 0.7821020483970642, 0.6927189230918884, 0.758472204208374, 0.6214988231658936, 0.3899267315864563, 0.48824360966682434, 0.46439963579177856, 0.4571281969547272, 0.42835763096809387, 0.38260531425476074, 0.5120084285736084, 0.6006606221199036, 0.3842446804046631, 0.7511336207389832, 0.48661959171295166]
