...
if cutedsl_available:
    ...
    @cute.jit
    def _add_epilogue(lhs, rhs, output):
        output_dtype = output.element_type
        for element in cutlass.range_constexpr(cute.size(lhs)):
            output[element] = (lhs[element] + rhs[element]).to(output_dtype)
...
def test_sm100_gemm_grouped_epilogue():
    torch.manual_seed(0)
    tokens, hidden, output_features = 33, 128, 96
    x = torch.randn(tokens, hidden, device="cuda", dtype=torch.bfloat16)
    lhs = torch.randn(output_features, hidden, device="cuda", dtype=torch.bfloat16) * hidden**-0.5
    rhs = torch.randn_like(lhs) * hidden**-0.5
    lhs_tiles = lhs.reshape(-1, EPILOGUE_TILE_SIZE, hidden)
    rhs_tiles = rhs.reshape(-1, EPILOGUE_TILE_SIZE, hidden)
    packed = torch.stack((lhs_tiles, rhs_tiles), dim=1).reshape(-1, hidden).contiguous()
    actual = torch.empty(tokens, output_features, device="cuda", dtype=torch.bfloat16)

    run_grouped_epilogue_gemm(x, packed, actual, _add_epilogue)

    expected = torch.nn.functional.linear(x, lhs) + torch.nn.functional.linear(x, rhs)
    torch.testing.assert_close(actual.float(), expected.float(), atol=0.05, rtol=0.03)
...
@pytest.mark.parametrize("entrypoint", ["run_epilogue_gemm", "run_grouped_epilogue_gemm"])
def test_sm100_gemm_custom_epilogues_guard_noncurrent_device(monkeypatch, entrypoint):
    ...
    getattr(gemm, entrypoint)(FakeTensor(), None, None, None)
