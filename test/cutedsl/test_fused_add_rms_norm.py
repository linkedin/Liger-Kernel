...
Parity oracle is the Triton implementation of the same ``fused_add_rms_norm``
dispatcher op (``liger-kernel``'s reference kernel). Checked: both returned
tensors (normed output and updated residual) plus ``dX``/``dR``/``dW``, across
dtypes and 2-D/3-D input shapes.
...
def test_cutedsl_fused_add_rms_norm_matches_triton(rows_shape, dtype, in_place):
    ...
    *batch, N = rows_shape
    ...
    eps = 1e-6
    atol_fwd, atol_bwd, rtol_bwd = _TOL[dtype] if dtype in _TOL else (1e-2, 1e-1, 1e-2)
...
@skip_no_cutedsl
@pytest.mark.parametrize("dtype", [torch.float32, torch.bfloat16])
def test_cutedsl_fused_add_rms_norm_3d_backward_does_not_raise(dtype):
    ...
    Y, S = dispatch("fused_add_rms_norm", X, R, W, 1e-6, in_place=False, impl="nvidia-cutedsl")
    assert Y.shape == (B, T, N)
    assert S.shape == (B, T, N)

    gY = torch.randn_like(Y)
    gS = torch.randn_like(S)
    torch.autograd.backward((Y, S), grad_tensors=(gY, gS))
