...
def test_correctness(B, T, H, V, scalar, dtype, beta, temperature, atol, rtol):
    device = "cuda"
    torch_lm_head_ce = TorchLMHeadJSD(
        H=H,
        V=V,
        dtype=dtype,
    ...
    liger_lm_head_ce = LigerLMHeadJSD(
        H=H,
        V=V,
        dtype=dtype,
        temperature=temperature,
    ...
    torch_lm_head_ce.student_lin.weight.data = (
        liger_lm_head_ce.student_lin.weight.data
    ) = torch.rand(V, H, device=device, dtype=dtype)
    torch_lm_head_ce.teacher_lin.weight.data = (
        liger_lm_head_ce.teacher_lin.weight.data
    ) = torch.rand(V, H, device=device, dtype=dtype)
    ...
    output1 = torch_lm_head_ce(_input1, teacher_input)
    output2 = liger_lm_head_ce(_input2, teacher_input)
    ...
    assert_verbose_allclose(
        torch_lm_head_ce.student_lin.weight.grad,
        liger_lm_head_ce.student_lin.weight.grad,
        atol=atol,
        rtol=rtol,
    )
