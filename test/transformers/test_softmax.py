...
def test_large_64k_softmax_correctness(dtype, atol, rtol):
    """Test softmax with 64k dimension to verify optimization works at scale"""
    torch.manual_seed(0)
    shape = (2, 65536)
    ...
    liger_softmax_layer = LigerSoftmax().to(device).to(dtype)
    liger_out = liger_softmax_layer(x2)
