...
def test_correctness_token_scaling():
    ...
    # Test gradients
    grad_output = torch.ones_like(loss_scaled)
...
def test_correctness_token_scaling_consistency():
    ...
    # Test gradients
    grad_output = torch.ones_like(loss_functional)
...
def test_correctness_token_scaling_functional():
    ...
    # Test gradients
    grad_output = torch.ones_like(y1)  # Use ones for simpler gradient checking
...
def test_correctness_token_scaling_module():
    ...
    # Compute standard cross entropy loss
    ce_loss_manual = torch.nn.functional.cross_entropy(
        logits, target, ignore_index=-100, reduction="none"
    )
