...
### sm90a → sm100a speedup (architectural rationale — Hopper run pending)

No Hopper GPU here (B200 only) → `TFLOPs_Hopper` SKIPs; H100 column **pending**:
...
**Why we expect a speedup (hypothesis).** B200 vs H100: bf16 peak ~2.27×, HBM bandwidth
~2.4×. mlp3 sits between the compute-bound (mlp5) and bandwidth-bound (mlp2) ends: at
large `T` it is compute-limited (so its sm90→sm100 speedup should lean toward the
**~2.27× compute ratio**), while at small `T` the REDUCE_ADD write dominates (leaning
toward the **~2.4× bandwidth ratio**). The `TmemLoadOp<64>` epilogue keeps the wide
`EpiChunkN=64` fragment off the register file, which matters more on B200 where the
accumulator is already TMEM-resident.
