...
def plot_data(df: pd.DataFrame, config: VisualizationsConfig):
    ...
    gpu_name_list = list(df.groupby('gpu_name', sort=False).groups.keys())

    # There are multiple data generated by different gpus, plotting the latest one
    if len(gpu_name_list) > 1:
        df = df[df["gpu_name"] == gpu_name_list[-1]]
