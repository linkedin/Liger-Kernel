...
third provider so users can reproduce end-to-end comparisons.
...
def _ensure_single_rank_tp_group():
    ...
    Megatron's fused_vocab_parallel_cross_entropy calls ``tp_group.rank()`` and
    ``tp_group.size()`` directly, so it requires a real ProcessGroup — None is
    not accepted. For a single-process benchmark we use the world group of
