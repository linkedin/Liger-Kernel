...
kernels, shipped as the standalone top-level package ``liger_cute_kernel``.
...
into ``liger_kernel``. The lck wheel:
  ...
  * builds the torch-free core ``libliger_cute_kernels.so`` + the per-torch
    ``_C`` pybind shim,
  * ships them as ``liger_cute_kernel/{_C.so, libliger_cute_kernels.so,
    libnvshmem_host.so}`` (plus its own ``__init__.py``),
  * is tagged with the CUDA + torch version as a PEP 440 local version, e.g.
    ``liger_cute_kernel-0.1.0+cu130.torch2.9.1-cp312-cp312-linux_x86_64.whl``.

``liger_kernel.ops.cute`` (from the liger wheel) imports ``liger_cute_kernel._C``
...
setup(
    ...
    packages=["liger_cute_kernel"],
    package_dir={"liger_cute_kernel": "liger_cute_kernel"},
    ext_modules=[CMakeExtension("liger_cute_kernel._C")],
