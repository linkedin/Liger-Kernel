...
def fused_linear_cross_entropy_forward(
    ...
    ce_stats = _make_ce_stats_buffer(target, ignore_index, ce_weight, reduction)
    ...
    # d(loss)/d(loss_1d) is 1 for the fused forward path that inlines CE backward.
    ce_grad_output_scalar = torch.ones((), dtype=torch.float32, device=device)
    ...
    for chunk_id in range(num_chunks):
        ...
        # When gradients run, allocate fp32 LSE rows; else forward never touches LSE — bind ``loss_1d_slice``
        # for the unused pointer (Triton cannot take ``None``).
        ...
        liger_cross_entropy_forward_kernel[(min(n_rows, num_cores),)](
            ...
            RETURN_LSE=input_requires_grad,
        ...
        if input_requires_grad:
            ...
            liger_cross_entropy_backward_kernel[(min(n_rows, num_cores),)](
                ...
                weight_ptr=ce_weight,
                lse_ptr=lse_1d_slice,
                grad_output_ptr=ce_grad_output_scalar,
